...
const ImagePost = (post) => {
  ...
  return (
    <div className="post border bg-white">
      ...
      <div>
        <img
          src={post.image ? post.image : defaultImagePost}
          className="w-full"
          alt="image_post"
        />
      </div>
      ...
      <div className="py-5">
        ...
        <div className="px-4 pt-2 pb-0">
          ...
          <div className="w-1/2">
            <p className={`w-2/3 text-sm ${isTruncate ? "truncate " : ""}`}>
              {post.title || "lorem ipsum scsaksaklnalksnsacsabckasnckn"}
            </p>
            <a
              className={`text-sm text-gray-400 ${
                isTruncate ? "inline" : "hidden"
              } cursor-pointer`}
              onClick={() => {
          ...
          <div>
            <a className="text-sm text-gray-400 cursor-pointer">
              View All {countComment || 245} Comments
            </a>
            {/* comments */}
            <div className="flex flex-row justify-between items-center py-0">
              ...
              >
                ...
                <a className="hover-decoration">
                  {post.user || "missingvivian"}
                </a>{" "}
                {post.comment || "<3<3<3<3"}{" "}
              </p>
      ...
      <div className="px-3 border-t py-3">
        <form className="flex flex-row justify-between items-center space-x-3">
          ...
          />
          <a className="text-sm font-semibold text-sky-500 cursor-pointer">
            Post
          </a>
        </form>
      </div>
